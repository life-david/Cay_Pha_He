...
-- LEGACY CAI GIA PHA STRUCTURE (ported from caygiapha.sql)
-- These tables preserve the original MySQL-style family tree model
-- alongside the main Supabase schema used by the application.
CREATE TABLE IF NOT EXISTS public.legacy_ca_nhan (
  ...
  ten VARCHAR(120),
  ho_ten VARCHAR(255),
  phan_bo_sung VARCHAR(255),
  ghi_chu VARCHAR(255),
  ghi_chu_2 VARCHAR(255),
  ...
  can_bo_sung BOOLEAN NOT NULL DEFAULT TRUE,
  source_file VARCHAR(255),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS public.legacy_hon_nhan (
  id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  family_group_id INT,
  spouse_1_node_id INT NOT NULL REFERENCES public.legacy_ca_nhan(node_id) ON DELETE CASCADE,
  spouse_2_node_id INT NOT NULL REFERENCES public.legacy_ca_nhan(node_id) ON DELETE CASCADE,
  loai_quan_he TEXT NOT NULL DEFAULT 'vo_chong' CHECK (loai_quan_he IN ('vo_chong', 'ket_hon', 'song_thu', 'khac')),
  ghi_chu VARCHAR(255),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS public.legacy_quan_he (
  id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  family_group_id INT NOT NULL,
  flag_1 INT,
  parent_1_node_id INT REFERENCES public.legacy_ca_nhan(node_id) ON DELETE SET NULL,
  flag_2 INT,
  parent_2_node_id INT REFERENCES public.legacy_ca_nhan(node_id) ON DELETE SET NULL,
  flag_3 INT,
  pos_x NUMERIC(10,2),
  pos_y NUMERIC(10,2),
  extra_1 INT,
...
-- Legacy structure indexes
CREATE INDEX IF NOT EXISTS idx_legacy_ca_nhan_family_group_id ON public.legacy_ca_nhan(family_group_id);
CREATE INDEX IF NOT EXISTS idx_legacy_ca_nhan_branch_order ON public.legacy_ca_nhan(branch_order);
CREATE INDEX IF NOT EXISTS idx_legacy_hon_nhan_family_group_id ON public.legacy_hon_nhan(family_group_id);
CREATE INDEX IF NOT EXISTS idx_legacy_hon_nhan_spouse_1_node_id ON public.legacy_hon_nhan(spouse_1_node_id);
CREATE INDEX IF NOT EXISTS idx_legacy_hon_nhan_spouse_2_node_id ON public.legacy_hon_nhan(spouse_2_node_id);
CREATE INDEX IF NOT EXISTS idx_legacy_quan_he_family_group_id ON public.legacy_quan_he(family_group_id);
CREATE INDEX IF NOT EXISTS idx_legacy_quan_he_parent_1_node_id ON public.legacy_quan_he(parent_1_node_id);
CREATE INDEX IF NOT EXISTS idx_legacy_quan_he_parent_2_node_id ON public.legacy_quan_he(parent_2_node_id);
...
DROP TRIGGER IF EXISTS tr_legacy_ca_nhan_updated_at ON public.legacy_ca_nhan;
CREATE TRIGGER tr_legacy_ca_nhan_updated_at BEFORE UPDATE ON public.legacy_ca_nhan FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();

DROP TRIGGER IF EXISTS tr_legacy_hon_nhan_updated_at ON public.legacy_hon_nhan;
CREATE TRIGGER tr_legacy_hon_nhan_updated_at BEFORE UPDATE ON public.legacy_hon_nhan FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();

DROP TRIGGER IF EXISTS tr_legacy_quan_he_updated_at ON public.legacy_quan_he;
CREATE TRIGGER tr_legacy_quan_he_updated_at BEFORE UPDATE ON public.legacy_quan_he FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();
