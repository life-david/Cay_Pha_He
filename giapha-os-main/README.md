<p align="center">
   <img src="https://raw.githubusercontent.com/homielab/giapha-os/main/public/icon.png" alt="Gia phả nhà Đỗ Văn Đạt Icon" width="100" height="100" style="border-radius: 22%; border: 0.5px solid rgba(0,0,0,0.1);" />
</p>

# Gia phả nhà Đỗ Văn Đạt
...
## Danh xưng hôn nhân & Thông gia (In‑laws)

Ứng dụng hỗ trợ nhận diện các danh xưng phát sinh từ quan hệ hôn nhân (in‑law) và giữa các phụ huynh của con cái đã kết hôn (thông gia).

- **Ông / Bà Thông gia**: Cha/mẹ của vợ hoặc chồng của con bạn — cha/mẹ của người con dâu/con rể. Khi con A của bạn kết hôn với con B của người khác, cha/mẹ của A và cha/mẹ của B gọi nhau là "ông/bà thông gia". Hệ thống sẽ tìm các cặp con đã kết hôn và xác định phụ huynh tương ứng.
...
- **Bác / Thím / Mợ / Chú**: Vợ/chồng của anh, chị, em của bố/mẹ — áp dụng tương tự như quan hệ huyết thống nhưng ghi nhận là "qua hôn nhân". Trong hệ thống này, chồng của cô hoặc dì được gọi là "Chú" thay vì "Dượng" theo tùy chỉnh vùng miền của gia đình.

Ghi chú dữ liệu để kết quả chính xác:
- Cần nhập đủ quan hệ `Bố/Mẹ - Con` và `Kết hôn`.
- `gender` phải chính xác để phân biệt Ông/Bà, Cô/Dì, Chú/Cậu.
- `birth_order` (thứ tự sinh) giúp phân định thứ bậc trong cùng thế hệ (Anh/Em).
...
> **Nạp dữ liệu legacy:** Nếu bạn muốn danh sách thành viên có dữ liệu ngay, hãy chạy thêm [docs/legacy-seed.sql](docs/legacy-seed.sql) sau khi tạo schema. File này sẽ nạp dữ liệu từ bản legacy vào `persons` và `relationships`.
