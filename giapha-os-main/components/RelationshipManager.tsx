...
import KinshipWarningModal from "./KinshipWarningModal";
...
interface RelationshipManagerProps {
  ...
  onStatsLoaded?: (stats: {
    ...
    paternalGreatGrandchildren?: number;
    maternalGreatGrandchildren?: number;
    sonInLaw: number;
...
export default function RelationshipManager({
  ...
  const getSurname = (fullName: string | null | undefined) => {
    if (!fullName) return "";
  ...
  // Debug helpers removed: siblings section disabled
  ...
  const [kinMessage, setKinMessage] = useState<string | null>(null);
  ...
  const fetchRelationships = useCallback(async () => {
    try {
      ...
      if (childrenIds.length > 0) {
        ...
        // Fetch parents of the in-laws (Ông/Bà thông gia)
        const spouseIds = formattedRels
          .filter((r) => r.direction === "child_in_law")
          .map((r) => r.targetPerson.id);

        if (spouseIds.length > 0) {
          try {
            const { data: spouseParents } = await supabase
              .from("relationships")
              .select(`*, parent:persons!person_a(*), child:persons!person_b(*)`)
              .in("type", ["biological_child", "adopted_child"])
              .in("person_b", spouseIds);

            if (spouseParents) {
              spouseParents.forEach((m) => {
                const parentPerson = m.parent;
                const childPerson = m.child; // this is the spouse (child of parentPerson)
                if (parentPerson && childPerson) {
                  const label =
                    (parentPerson.gender === "male" ? "Ông" : "Bà") +
                    ` thông gia (phụ huynh của ${childPerson.full_name})`;

                  formattedRels.push({
                    id: m.id + "_thonggia",
                    type: "marriage",
                    direction: "in_law_parent",
                    targetPerson: parentPerson,
                    note: label,
                  });
                }
              });
            ...
            console.error("Error fetching spouse parents (thong gia):", err);
          }
        }
      ...
      if (onStatsLoaded) {
        ...
        // Fetch Grandchildren mapping and compute grandchildren + great-grandchildren
        let paternalGrandchildren = 0;
        let maternalGrandchildren = 0;
        let paternalGreatGrandchildren = 0;
        let maternalGreatGrandchildren = 0;
        if (childrenIds.length > 0) {
          ...
          if (grandchildrenData) {
            ...
            // Build map: grandchildId -> parentChildId (which child of the person is the parent of this grandchild)
            const grandchildParentMap = new Map<string, string>();
            const grandchildrenIds: string[] = [];
            for (const row of grandchildrenData) {
              const parentId = row.person_a;
              const grandchildId = row.person_b;
              if (grandchildId && parentId) {
                grandchildParentMap.set(grandchildId, parentId);
            ...
            // Fetch great-grandchildren (chắt) - children of grandchildren
            if (grandchildrenIds.length > 0) {
              const { data: greatData } = await supabase
                .from("relationships")
                .select("person_a, person_b")
                .in("type", ["biological_child", "adopted_child"])
                .in("person_a", grandchildrenIds);

              if (greatData) {
                for (const gRow of greatData) {
                  const grandchildId = gRow.person_a;
                  const parentChildId = grandchildParentMap.get(grandchildId);
                  if (!parentChildId) continue;
                  if (maleChildrenIds.includes(parentChildId)) {
                    paternalGreatGrandchildren += 1;
                  } else if (femaleChildrenIds.includes(parentChildId)) {
                    maternalGreatGrandchildren += 1;
                  }
                }
              }

              // Sibling computation moved below to run regardless of grandchildren presence
            }
          }
        }
        ...
        onStatsLoaded({
          ...
          paternalGreatGrandchildren,
          maternalGreatGrandchildren,
          sonInLaw,
          daughterInLaw,
        });
      }

      // Compute siblings (Anh/Chị/Em ruột) by finding other children of this person's parents
      try {
        ...
        if (parentIds.length > 0) {
          const { data: siblingsRels, error: siblingsErr } = await supabase
            .from("relationships")
          ...
          if (siblingsErr) {
            console.error("Error fetching siblings:", siblingsErr);
          } else if (siblingsRels) {
            const seen = new Set<string>();
            siblingsRels.forEach((sr: any) => {
              const child = sr.target;
              if (!child || child.id === personId) return;
              if (seen.has(child.id)) return;
              seen.add(child.id);

              formattedRels.push({
                id: `sibling_${child.id}`,
                type: sr.type || "biological_child",
                direction: "sibling",
                targetPerson: child,
                note: null,
              });
        ...
        console.error("Sibling computation failed:", err);
      }
  ...
  const handleAddRelationship = async () => {
    ...
    try {
      ...
      // If creating a marriage, check kinship based on existing relationships (not name)
      if (type === "marriage") {
        try {
          // fetch target and all data to compute kinship
          const [{ data: targetPersonData }, { data: allPersons }, { data: allRels }] = await Promise.all([
            supabase.from("persons").select("*").eq("id", selectedTargetId).single(),
            supabase.from("persons").select("*"),
            supabase.from("relationships").select("*"),
          ]);

          const kin = computeKinship(
            {
              id: person.id,
            ...
            (targetPersonData as any) ?? ({} as any),
            (allPersons as any) ?? [],
            (allRels as any) ?? [],
          );

          // Theo quy định: cảnh báo khi quan hệ gần trong vòng 3 đời (<= 3 đời)
          if (
            kin &&
            kin.distance >= 0 &&
            typeof kin.depthA === "number" &&
            typeof kin.depthB === "number" &&
            Math.max(kin.depthA, kin.depthB) <= 3
          ) {
            const msg = `Phát hiện quan hệ gần (<=3 đời) giữa "${person.full_name}" và "${targetPersonData?.full_name || selectedTargetId}" (${kin.description}).`;
            setKinMessage(msg);
            setPendingInsert({ personA, personB, type, note: newRelNote ? newRelNote : null });
            setShowKinModal(true);
            setProcessing(false);
            return;
          }
        } catch (err) {
          console.error("Warning kinship check failed:", err);
        }
      }

      if (newRelDirection === "sibling") {
        // Find parents of the current person and attach the selected target as their child
        const { data: parentRels, error: parentErr } = await supabase
          .from("relationships")
          .select("person_a")
          .in("type", ["biological_child", "adopted_child"])
          .eq("person_b", personId);
        ...
        if (!parentRels || parentRels.length === 0) {
          setError("Không thể thêm anh/chị/em: chưa có thông tin cha/mẹ để gán chung.");
          setProcessing(false);
          return;
        }
        ...
        for (const pr of parentRels) {
          const parentId = (pr as any).person_a;
          try {
            const { error: insErr } = await supabase.from("relationships").insert({
              person_a: parentId,
              person_b: selectedTargetId,
              type: "biological_child",
              note: newRelNote ? newRelNote : null,
            });
            if (insErr) {
              // Postgres unique violation code may vary; treat as non-fatal if it's conflict
              const code = (insErr as any).code || (insErr as any).status;
              if (code === "23505" || code === 409) {
                // already exists -> not an error for our purpose
                continue;
              }
              // otherwise remember first error
              if (!firstError) firstError = insErr;
            } else {
              anyInserted += 1;
            }
          } catch (e) {
            if (!firstError) firstError = e;
            console.error("Error inserting sibling relation:", e);
          }
        }

        if (firstError && anyInserted === 0) {
          // Show a useful error to user if nothing was inserted
          setError(
            "Không thể thêm anh/chị/em: " + (firstError?.message || JSON.stringify(firstError)),
          );
          setProcessing(false);
          return;
        }

        // After attaching to parents, ensure target generation/is_in_law updated
        try {
          ...
          if (targetPerson && (targetPerson.generation == null || targetPerson.is_in_law == null)) {
            ...
            if (targetPerson.generation == null && person.generation != null) {
              // sibling should have same generation
          ...
          console.error("Failed to auto-update target person properties after sibling insert", err);
        }
      } else {
  ...
  const performInsertRelationship = async (personA: string, personB: string, type: RelationshipType, note: string | null) => {
    setProcessing(true);
    try {
      const { error } = await supabase.from("relationships").insert({
        person_a: personA,
      ...
      try {
        ...
        if (
          ...
          if (targetPerson.generation == null && person.generation != null) {
            // best-effort: assume relation direction preserved
            updates.generation = person.generation;
          }

          if (targetPerson.is_in_law == null) updates.is_in_law = person.is_in_law === true ? false : true;
  ...
  const handleConfirmKinModal = async () => {
    if (!pendingInsert) {
      setShowKinModal(false);
      return;
    }
    const { personA, personB, type, note } = pendingInsert;
    setShowKinModal(false);
    setPendingInsert(null);
    await performInsertRelationship(personA, personB, type, note);
  };

  const handleCancelKinModal = () => {
    setShowKinModal(false);
    setPendingInsert(null);
  };
  ...
  const handleQuickAddSpouse = async () => {
    ...
    // Warn if same surname
    const surnameA = getSurname(person.full_name);
    const surnameB = getSurname(newSpouseName);
    if (surnameA && surnameB && surnameA === surnameB) {
      const ok = confirm(
        "Cảnh báo: Hai người cùng họ. Bạn có chắc muốn tiếp tục thêm?",
      );
      if (!ok) return;
    }
  ...
  return (
    <div className="space-y-6">
      <KinshipWarningModal
        open={showKinModal}
        message={kinMessage ?? ""}
        onConfirm={handleConfirmKinModal}
        onCancel={handleCancelKinModal}
      />
      {/* List Sections */}
      {["parent", "sibling", "spouse", "child", "child_in_law", "in_law_parent"].map((group) => {
        ...
        if (group === "in_law_parent") title = "Ông / Bà Thông gia";

        if (items.length === 0 && !isAdmin && group !== "sibling") return null; // Hide empty sections for members (except sibling which may show a hint)
        ...
        return (
          ...
          >
            ...
            {items.length > 0 ? (
              <ul className="space-y-3">
                {items.map((rel) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex flex-col">
                        <span className="text-stone-900 font-medium text-sm">
                          ...
                          {rel.direction === 'sibling' && (() => {
                            const pOrder = person.birth_order;
                            const tOrder = rel.targetPerson.birth_order;
                            const pYear = person.birth_year;
                            const tYear = rel.targetPerson.birth_year;
                            let older: boolean | null = null;
                            if (typeof pOrder === 'number' && typeof tOrder === 'number') {
                              if (tOrder < pOrder) older = true;
                              else if (tOrder > pOrder) older = false;
                            } else if (typeof pYear === 'number' && typeof tYear === 'number') {
                              if (tYear < pYear) older = true;
                              else if (tYear > pYear) older = false;
                            }

                            let label = '';
                            if (older === true) {
                              label = rel.targetPerson.gender === 'male' ? 'Anh' : rel.targetPerson.gender === 'female' ? 'Chị' : 'Anh/Chị';
                            } else if (older === false) {
                              label = rel.targetPerson.gender === 'male' ? 'Em trai' : rel.targetPerson.gender === 'female' ? 'Em gái' : 'Em';
                            } else {
                              // fallback: use gendered older form
                              label = rel.targetPerson.gender === 'male' ? 'Anh' : rel.targetPerson.gender === 'female' ? 'Chị' : '';
                            }

                            return label ? <span className="text-xs text-stone-400 ml-2">({label})</span> : null;
                          })()}
                        </span>
                        {rel.note && (
                    ...
                    {canEdit && rel.direction !== "child_in_law" && rel.direction !== "in_law_parent" && rel.direction !== "sibling" && (
                      <button
              ...
              <>
                {group === "sibling" && relationships.filter((r) => r.direction === "parent").length > 0 ? (
                  <p className="text-xs text-stone-400 italic">
                    Chưa tìm thấy Anh/Chị/Em — có thể các quan hệ con của cha/mẹ chưa đầy đủ trong dữ liệu.
                  </p>
                ) : (
      ...
      {/* Admin debug info removed — sibling detection relies only on parents' children */}
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setNewRelDirection(
                    e.target.value as "parent" | "child" | "spouse" | "sibling",
                  )
                }
              ...
              >
                ...
                <option value="sibling">Người này là Anh/Chị/Em của...</option>
              </select>
            </div>
