...
export default function FamilyStats({
  ...
  const scopeOptions = useMemo(() => {
    ...
    relationships.forEach((r) => {
      if (r.type === "biological_child" || r.type === "adopted_child") {
        const children = childMap.get(r.person_a) ?? [];
        children.push(r.person_b);
        childMap.set(r.person_a, children);
        parentIds.add(r.person_b);
      }
    });
    ...
    return persons
      .filter(
        (p) =>
          p.generation != null && p.generation >= 1 && p.generation <= 3,
      )
      .sort((a, b) => a.full_name.localeCompare(b.full_name, "vi"))
      .map((p) => ({
        id: p.id,
        label: `${p.full_name}${p.generation != null ? ` - Đời ${p.generation}` : ""}`,
      }));
  ...
  const scopedData = useMemo(() => {
    ...
    relationships.forEach((r) => {
      if (r.type === "biological_child" || r.type === "adopted_child") {
        ...
        const spousesA = spouseMap.get(r.person_a) ?? [];
        spousesA.push(r.person_b);
        spouseMap.set(r.person_a, spousesA);
        const spousesB = spouseMap.get(r.person_b) ?? [];
        spousesB.push(r.person_a);
        spouseMap.set(r.person_b, spousesB);
      }
    });

    const includedIds = new Set<string>([scope]);
    const queue = [scope];
    ...
    while (queue.length > 0) {
      ...
      for (const spouseId of spouseMap.get(currentId) ?? []) {
        includedIds.add(spouseId);
      }
    }

    const scopedPersons = persons.filter((p) => includedIds.has(p.id));
    const scopedRelationships = relationships.filter(
      (r) => includedIds.has(r.person_a) && includedIds.has(r.person_b),
    );

    const scopePerson = persons.find((p) => p.id === scope);

    return {
      persons: scopedPersons,
      relationships: scopedRelationships,
      title: scopePerson ? scopePerson.full_name : "Đã chọn",
    };
  }, [persons, relationships, scope]);

  const stats = useMemo(() => {
    const total = scopedData.persons.length;

    // Gender
    const male = scopedData.persons.filter((p) => p.gender === "male").length;
    const female = scopedData.persons.filter((p) => p.gender === "female").length;

    // In-laws
    const daughtersInLaw = scopedData.persons.filter(
      (p) => p.is_in_law && p.gender === "female",
    ).length;
    const sonsInLaw = scopedData.persons.filter(
      (p) => p.is_in_law && p.gender === "male",
    ).length;

    // Deceased
    const deceased = scopedData.persons.filter((p) => p.is_deceased).length;

    // First-born (con trưởng)
    const firstBorn = scopedData.persons.filter((p) => p.birth_order === 1).length;

    // Married / unmarried (based on marriage relationships)
    const marriedIds = new Set<string>();
    scopedData.relationships
      .filter((r) => r.type === "marriage")
      .forEach((r) => {
        marriedIds.add(r.person_a);
        marriedIds.add(r.person_b);
      });
    const married = scopedData.persons.filter((p) => marriedIds.has(p.id)).length;
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white/80 border border-stone-200/60 rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-stone-400">
            Phạm vi thống kê
          </p>
          <p className="text-sm text-stone-600 mt-1">
            Chọn một người gốc để xem số liệu theo nhánh gia đình, hoặc xem toàn bộ dòng họ.
          </p>
        </div>
        <div className="min-w-[240px] sm:min-w-[320px]">
          <select
            ...
            className="w-full rounded-xl border border-stone-200 bg-white px-3 py-2.5 text-sm font-medium text-stone-700 shadow-sm outline-none transition focus:border-amber-300 focus:ring-2 focus:ring-amber-100"
          >
