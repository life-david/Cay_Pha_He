...
export default function PersonCard({ person }: PersonCardProps) {
  const { setMemberModalId, setRootId, view: currentView, setView } = useDashboard();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-4 relative z-10">
        <div className="relative">
          <div
            onClick={(e) => {
              e.stopPropagation();
              // Default behavior: select this person as the dashboard root (like choosing from selector)
              // Hold Shift or Meta/Cmd to open the branch modal instead.
            ...
            role="button"
            tabIndex={0}
            className={`size-14 sm:size-16 rounded-full flex items-center justify-center text-xl font-bold text-white overflow-hidden shrink-0 shadow-lg ring-2 ring-white transition-transform duration-300 group-hover:scale-105 ${getAvatarBg(person.gender)}`}
          >
            {person.avatar_url ? (
              <Image
                unoptimized
      ...
      {showBranch && (
        <FamilyBranchModal open={showBranch} personId={person.id} onClose={() => setShowBranch(false)} />
      )}
    </>
  );
}
