...
import FamilyBranchModal from "./FamilyBranchModal";
...
export default function MemberDetailContent({
  ...
  const { setRootId, view: currentView, setView } = useDashboard();
      ...
      <div className="h-28 sm:h-36 bg-linear-to-r from-stone-200 via-stone-100 to-stone-200 relative shrink-0">
        ...
        >
          <div
            ...
            tabIndex={0}
            onClick={(e) => {
              // Shift+click sets this person as dashboard root; plain click opens branch modal
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" || e.key === " ") setShowBranchModal(true);
            }}
            className={`cursor-pointer h-24 w-24 sm:h-32 sm:w-32 rounded-full border-4 sm:border-[6px] border-white flex items-center justify-center text-3xl sm:text-4xl font-bold text-white overflow-hidden shadow-xl shrink-0
             ${
               person.gender === "male"
            ...
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
              ...
              {relStats &&
                (relStats.biologicalChildren > 0 ||
                  ...
                  (relStats.paternalGrandchildren ?? 0) > 0 ||
                  (relStats.maternalGrandchildren ?? 0) > 0 ||
                  (relStats.paternalGreatGrandchildren ?? 0) > 0 ||
                  (relStats.maternalGreatGrandchildren ?? 0) > 0) && (
                  <motion.div
                    layout
                    variants={itemVariants}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                      ...
                      {/* Great-grandchildren (Chắt) - separate yellow card */}
                      {((relStats.paternalGreatGrandchildren ?? 0) +
                        (relStats.maternalGreatGrandchildren ?? 0)) > 0 && (
                        <div className="bg-stone-50/80 rounded-xl p-3 border border-stone-100 flex flex-col group hover:bg-stone-100/80 transition-colors">
                          <div className="flex items-center gap-3 mb-2">
                            <div className="p-2 bg-amber-100/60 text-amber-600 rounded-lg group-hover:bg-amber-100 transition-colors">
                              <Baby className="size-4" />
                            </div>
                            <p className="text-[10px] font-bold text-stone-500 uppercase tracking-widest">
                              Chắt
                          ...
                          <div className="space-y-1 mt-auto w-full pt-1 border-t border-stone-200/50">
                            {(relStats.paternalGreatGrandchildren ?? 0) > 0 && (
                              <div className="flex items-center justify-between text-xs">
                                ...
                                <span className="font-bold text-stone-700">
                                  {relStats.paternalGreatGrandchildren}
                                </span>
                              </div>
                            )}
                            {(relStats.maternalGreatGrandchildren ?? 0) > 0 && (
                              <div className="flex items-center justify-between text-xs">
                                ...
                                <span className="font-bold text-stone-700">
                                  {relStats.maternalGreatGrandchildren}
                                </span>
                              </div>
                            )}
                          </div>
